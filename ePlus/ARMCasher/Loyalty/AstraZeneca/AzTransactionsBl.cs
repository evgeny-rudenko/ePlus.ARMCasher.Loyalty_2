...
// Type: ePlus.ARMCasher.Loyalty.AstraZeneca.AzTransactionsBl
...
namespace ePlus.ARMCasher.Loyalty.AstraZeneca
{
  internal class AzTransactionsBl
  {
    ...
    {
      string tableName = "#CHEQUE_ITEM_TRANSACTION_AZ";
          ...
          using (DataTable dataTable = this.loader.CreateDataTable(transactions))
          {
            using (SqlCommand command1 = connection.CreateCommand())
            {
              using (SqlCommand command2 = connection.CreateCommand())
              {
                ...
                command2.CommandText = "EXEC USP_CHEQUE_ITEM_TRANSACTION_AZ_SAVE";
    ...
    public IEnumerable<CHEQUE_ITEM_TRANSACTION> GetListUnconfirmed() => (IEnumerable<CHEQUE_ITEM_TRANSACTION>) (this.loader.GetList(this.dataService.Execute(string.Format("EXEC USP_CHEQUE_ITEM_TRANSACTION_AZ_LOAD")).Tables[0]) ?? new List<CHEQUE_ITEM_TRANSACTION>());
  }
}
